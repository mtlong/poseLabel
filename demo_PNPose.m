...
% 
% for i=1:F
%     img = imread(['~/databag/mpii_human_pose/images/' MPI_anno(i).image]);    
%     ptsX = cell2mat({MPI_anno(i).points.x});
%     ptsY = cell2mat({MPI_anno(i).points.y});
%     figure(1);
%     clf;
%     imshow(img);
%     hold on;
%     plot(ptsX,ptsY,'ro','MarkerSize',5,'MarkerFaceColor','r');
%     pause(0.1);    
% end